sap.ui.define(
  ...
  function (BaseController, JSONModel, formatter, mobileLibrary, Fragment) {
    ...
    function _calculateOrderTotal(fPreviousTotal, oCurrentContext) {
      var fItemTotal =
        oCurrentContext.getObject().Quantity *
        oCurrentContext.getObject().UnitPrice;
        ...
        $.ajax({
          ...
          .done(
            function (oResult) {
              ...
              const thumbnails = that.getView().byId("thumbnails");
              ...
              $.ajax({
                ...
                .done(
                  function (oResult) {
                    ...
                    $.get("/paint/Categories?$filter=Language eq 'TR'")
                      .done(
                        function (oResult) {
                          for (var i = 0; i < 3; i++) {
                          ...
                          for (var i = 0; i < results.length; i++) {
                            ...
                            var oSegmentButtonItem = new sap.m.Button({
                              text: txt,
                            ...
                            oSegmentButtonItem.addStyleClass(
                              "myplaymusicbutton"
                            );
                            this.getView()
                              .byId("rd1")
                              .addItem(oSegmentButtonItem);
                          }
                          sap.ui.core.BusyIndicator.hide();

                          var clickedItems = this.getView()
                            .getModel("main")
                            .getData().ClickedItems;
                        }.bind(this)
                      )
                      .fail(function () {});
                  }.bind(this)
